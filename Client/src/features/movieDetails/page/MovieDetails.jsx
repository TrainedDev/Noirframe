...
import { ReviewsLoading } from "@/components/ui/HeroComponentSkeleton";
...
import { getMovieReviews } from "@/features/review/reviewSlice";
...
const MovieDetails = () => {
  ...
  const {
    ...
    error: { moviePageError, similarError, upcomingError, recommendError },
  ...
  useEffect(() => {
    const getMovie = async () => {
      await dispatch(getMoviesPage(id)).unwrap();
  ...
  const { movieTrailer: moviesTagData, trailerData: movieTrailer } =
    movieDetails;
  const movieTags = moviesTagData || movieDetails?.genres;
  ...
  return (
    ...
    `}
          ...
          <img
            ...
            className="w-full h-screen bg-black object-center object-cover "
          />
        )}
      </section>
